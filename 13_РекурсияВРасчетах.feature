...
Сценарий: 13.01 Создаем вид отчета с рекурсией расчета показателей
	...
	* Создаем бланк сводной таблицы
		И Я для вида отчета 'ВА - Трансформация периодов' я создаю бланк сводной таблицы по умолчанию с отборами 'Статьи ДДС'
...
Сценарий: 13.02 Вводим данные в сводную таблицу, получаем ошибку рекурсивного расчета
	...
	* Вводим значение
		...
		ИначеЕсли '$$ЯзыкИнтерфейса$$ = "En"' Тогда
			Тогда табличный документ 'ПолеТабличногоДокумента' равен:
				| 'Errors'                                                                                                                                                       | '' | ''                                                                                                                       |
				| 'Recursion detected in relationships when calculating an indicator: "Приемник сумма".\nReview the calculation formulas and save the calculation rule again.'   | '' | '{"S","Calculation Tree:\n	Итог сумма\n		Накопление сумма\n			Приемник сумма\n				Источник сумма\n				Итог сумма"}'      |
				| 'Recursion detected in relationships when calculating an indicator: "Накопление сумма".\nReview the calculation formulas and save the calculation rule again.' | '' | '{"S","Calculation Tree:\n	Приемник сумма\n		Источник сумма\n		Итог сумма\n			Накопление сумма\n				Приемник сумма"}'    |
				| 'Recursion detected in relationships when calculating an indicator: "Итог сумма".\nReview the calculation formulas and save the calculation rule again.'       | '' | '{"S","Calculation Tree:\n	Накопление сумма\n		Приемник сумма\n			Источник сумма\n			Итог сумма\n				Накопление сумма"}' |
...
Сценарий: 13.03 Перенастраиваем структуру вида отчета
	...
	* Накопление
		...
		И в табличном документе 'ТабДокВыборПоказателя' я перехожу к ячейке 'Накопление_Сумма'
...
Сценарий: 13.04 Проверяем расчет в сводной таблице
	...
	* Вводим значения
		...
		* Сверяем результат
			Если '$$ЯзыкИнтерфейса$$ <> "Ru"' Тогда
				// ДОДЕЛАТЬ: Еще нет изменний для сложных рекурсивных расчетов
				Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен:
					| 'ВА - Трансформация периодов (сводная таблица)' | 'Январь 2021 г.' | 'Февраль 2021 г.' | 'Март 2021 г.' | 'Итого' |
