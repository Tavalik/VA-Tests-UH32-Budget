...
Сценарий: 13.02 Вводим данные в сводную таблицу, получаем ошибку рекурсивного расчета
	...
	* Вводим значение
		Когда открылось окно "Сводная таблица: ВА - Трансформация периодов(cводная таблица)( Основной) *"
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке 'R4C2'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я делаю двойной клик на текущей ячейке
		...
		Когда открылось окно "Сводная таблица: ВА - Трансформация периодов(cводная таблица)( Основной) *"
		И Я закрываю окно "Сводная таблица: ВА - Трансформация периодов(cводная таблица)( Основной) *"
	...
	* Вводим значения
		...
		* Сверяем результат
			Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен:
				| 'ВА - Трансформация периодов(cводная таблица)' | 'Январь 2021 г.' | 'Февраль 2021 г.' | 'Март 2021 г.' | 'Итого' |
				| 'ВА - Трансформация периодов(cводная таблица)' | 'Сумма'          | 'Сумма'           | 'Сумма'        | 'Сумма' |
