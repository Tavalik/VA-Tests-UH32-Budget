...
Сценарий: 02.02 Создание отчета с ручным вводом - 'ВА - Количество'
	...
	* Редактируем структуру отчета
	...
	И Я Для вида отчета 'ВА - Количество' я создаю бланк по умолчанию
...
Сценарий: 02.03 Создание экземпляра отчета - 'ВА - Количество'
	...
	И Я создаю экземпляр отчета для вида отчета 'ВА - Количество' сценарий 'ВА - Основной сценарий' период '01.01.2021' '31.03.2021' периодичность "Месяц" организация 'Меркурий ООО' проект '' аналитики '' '' '' '' '' '' 
...
Сценарий: 02.04 Создание отчета с загрузкой из РС 'ВА - Цена'
	...
	* Вводим формулы расчета
	...
	И Я Для вида отчета 'ВА - Цена' я создаю бланк по умолчанию
...
Сценарий: 02.05 Создание экземпляра отчета - 'ВА - Цена'
	...
	И Я создаю экземпляр отчета для вида отчета 'ВА - Цена' сценарий 'ВА - Основной сценарий' период '01.01.2021' '31.03.2021' периодичность "Месяц" организация 'Меркурий ООО' проект '' аналитики '' '' '' '' '' ''
...
Сценарий: 02.06 Создание отчета с рассчитываемыми показателями - 'ВА - Продажи'
	...
	* Вводим формулы расчета
		...
		И Я выбираю показатель с кодом 'Номенклатура_Количество' вида отчета с кодом 'ВАКоличество'
		Тогда открылось окно "Конструктор отчета *"
		И я нажимаю на кнопку с именем 'ЗаписатьИСвернуть'
		Тогда открылось окно "Конструктор отчета"
		...
		И в табличном документе 'ПолеТабличногоДокументаМакет' я делаю двойной клик на текущей ячейке
		И я нажимаю на кнопку с именем 'СсылкаНаПоказатель1'
		И Я выбираю показатель с кодом 'Номенклатура_Цена' вида отчета с кодом 'ВАЦена'
		Тогда открылось окно "Конструктор отчета *"
		И я нажимаю на кнопку с именем 'ЗаписатьИСвернуть'
		Тогда открылось окно "Конструктор отчета"
	...
	И Я Для вида отчета 'ВА - Продажи' я создаю бланк по умолчанию
...
Сценарий: 02.07 Создание экземпляра отчета - 'ВА - Продажи'	- пишем черновые версии
	...
	И Я создаю экземпляр отчета для вида отчета 'ВА - Продажи' сценарий 'ВА - Основной сценарий' период '01.01.2021' '31.03.2021' периодичность "Месяц" организация 'Меркурий ООО' проект '' аналитики '' '' '' '' '' ''
	И я запоминаю текущее окно как 'ЗаголовокОкна'
	...
	* Ввод показателей
		...
		Иначе
			Тогда элемент формы с именем 'ПолеТекстовогоДокументаПроцедура' стал равен 
				...
				|'ArrayIndicators.Add(\"ВАПродажи:Номенклатура_Количество\");'|
		...
		Иначе
			Тогда элемент формы с именем 'ПолеТекстовогоДокументаПроцедура' стал равен 
				|' ArrayIndicators = New Array;'|
				|'ArrayIndicators.Add(\"ВАЦена:Номенклатура_Цена\");'|
				|'tValuesIndicators = UGetIndicatorsValue(New Structure(\"IndicatorsReports\", ArrayIndicators));'|
				|''|
		И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст ' '	
		...
		Иначе
			...
			* УстановитьЗначениеПоказателя
				...
				Тогда элемент формы с именем 'ПолеТекстовогоДокументаПроцедура' стал равен 
					...
					|'	[<ReportPeriod>],'|
					|'	[<Analytics1>], [<Analytics2>], [<Analytics3>], [<Analytics4>], [< Analytics5>], [< Analytics6>],'|
					|'	[<AnalyticsCurrency>]);'|	
				И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст ' '
			* ДатаПринадлежитПериоду
				И в таблице 'Определения' я перехожу к строке:
			...
			* ВернутьМассивОрганизацийОсуществляющихКонтрольПериметра
				И в таблице 'Определения' я перехожу к строке:
					...
					| 'UPerimeterControllingEntities()' |
				И в таблице 'Определения' я выбираю текущую строку
				Тогда элемент формы с именем 'ПолеТекстовогоДокументаПроцедура' стал равен ' UPerimeterControllingEntities();'
				И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст ' '
			* ОчиститьВсе
				И в таблице 'Определения' я перехожу к строке:
					| "Метод"       |
	...
	* Вводим текст процедуры
		...
		Иначе
			И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст 
				...
				|'  Справочники.ПоказателиОтчетов.НайтиПоКоду(\"Номенклатура_Цена\",True,,CalcStructure.ReportKind));'|
