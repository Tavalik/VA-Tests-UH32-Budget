...
Сценарий: 01.06 Работа в контрукторе отчета - Ввод формул
	...
	* Вводим формулу через конструктор - 'СтрокаСинтетика - Сумма'
		...
		И Я выбираю показатель с кодом 'СТРКСинтетика_Количество1' вида отчета с кодом 'ВАРучнойВводПоВсемАналитикам'
		Тогда открылось окно "Конструктор отчета*"
		И я нажимаю на кнопку с именем 'КнопкаУмножить'
		И я нажимаю на кнопку с именем 'СсылкаНаПоказатель1'
...
Сценарий: 01.09 Проверяем движения документа
	...
	* Проверяем движения документа
		...
		// ДОДЕЛАТЬ, Временная заглушка для CPMWE, пока не решился вопрос сравнения движений
		Если 'НЕ $$ЭтоPerform$$' Тогда
...
Сценарий: 01.10 Копирование экземпляра отчета
	...
	* Проверяем движения документа
		...
		Тогда открылось окно "Плоская таблица значений показателей"
		И я жду когда в табличном документе 'ОтчетТабличныйДокумент' заполнится ячейка 'R2C1' в течение 30 секунд
		// ДОДЕЛАТЬ, Временная заглушка для CPMWE, пока не решены проблемы перевода макетов
		Если 'НЕ $$ЭтоPerform$$' Тогда
...
Сценарий: 01.20 Переходим в режим отображения данных
	...
	* Тестируем кнопки настроек
		...
		Если '$$ЭтоPerform$$' Тогда
			// ДОДЕЛАТЬ: В Перформе еще нет новой таблицы колонок
