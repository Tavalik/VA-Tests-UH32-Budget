...
Сценарий: 18.13 Проверка корректировок значений покзаталей для "Бюджетирование2"
...
Сценарий: 18.14 Проверка экземпляров отчетов для "Бюджетирование2"
	...
	* Создаем отчет по организации, к которой нет доступа
		И Я создаю экземпляр отчета для вида отчета "ВА - Проверка настроек доступа (запись)" сценарий "ВА - Основной сценарий" период '01.01.2024' '31.03.2024' периодичность "Месяц" организация "Система ООО" проект "ВА - Основной проект" аналитики "Администрация" '' '' '' '' ''
		Тогда в логе сообщений TestClient есть строки:
			|"У пользователя Бюджетирование2\nотсутствуют права на изменение экземпляров отчетов с бланком ВА - Проверка настроек доступа (нет) по организации Система ООО"|
...
Сценарий: 18.15 Проверка СТ для пользователя "Бюджетирование2"
...
Сценарий: 18.16 Проверка корректировок значений покзаталей для "Бюджетирование3"
...
Сценарий: 18.17 Проверка экземпляров отчетов для "Бюджетирование3"
	...
	* Открываем документ с правами на чтение
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '\\Макеты\18\ВА_ПроверкаНастроекДоступа_1.mxl'		
		Когда открылось окно '$ЗаголовокОкна$'
		И я закрываю окно '$ЗаголовокОкна$'
		
Сценарий: 18.18 Проверка СТ для пользователя "Бюджетирование3"
...
Сценарий: 18.19 Тест интерфейса для пользователя "Бюджетирование3" - Панель разделов и функций
	...
	* Панель функций
		...
		И панель функций равна
			...
			| "Мониторы ключевых показателей" | // Сбалансированная система показателей
			...
			| "Копирование данных показателей отчетов" | // Сервис
			| "Сравнение табличных документов" |

	И я закрываю сеанс текущего клиента тестирования					

Сценарий: 18.20 Тест интерфейса для пользователя "Бюджетирование1" - Панель разделов и функций
...
Сценарий: 18.21 Тест интерфейса для пользователя "Бюджетирование1" - Персональные настройки
...
Сценарий: 18.22 Тест интерфейса для пользователя "Бюджетирование1" - Виды отчетов, экземпляры отчетов, сводная таблица
	...
	* Заполнение
		...
		И я нажимаю на кнопку с именем 'ФормаПротоколОбработки'
		Тогда открылось окно "Экземпляры отчетов"
		И в таблице 'Список' я выбираю текущую строку
		И Открылся экземпляр отчета для вида отчета "ВА - Проверка настроек доступа (запись)" валюта "RUB" организация "Меркурий ООО" сценарий "ВА - Основной сценарий" периодичность 'Месяц' проект "ВА - Основной проект" аналитики "Администрация" '' '' '' '' '' 
		И я закрываю окно '$ЗаголовокОкна$'
...
Сценарий: 18.23 Тест интерфейса для пользователя "Бюджетирование1" - Управление отчетным периодом
	...
	* Проверка формы списка
		...
		И элемент формы с именем 'МатрицаОтчетов' отсутствует на форме
	...
	* Проверка формы элемента
		...
		И элемент формы с именем 'ОткрытьВидыОтчетовДляУстановкиЛимитов' отсутствует на форме
		И элемент формы с именем 'ДатаНачалаПроцесса' отсутствует на форме
...
Сценарий: 18.24 Тест интерфейса для пользователя "Бюджетирование1" - Корректировка значений показателей
...
Сценарий: 18.25 Тест интерфейса для пользователя "Бюджетирование1" - Шаблон корректировка значений показателей
	...
	Тогда таблица 'ИспользуемыеВидыОтчетов' стала равной по шаблону:
		| "N" | "Вид отчета"                              |
		| "1" | "<Объект не найден> (*)"                  |
		| "2" | "ВА - Проверка настроек доступа (запись)" |
	...
	Тогда таблица 'ИспользуемыеВидыОтчетов' стала равной:
		| "N" | "Вид отчета"                              |
		| "1" | "ВА - Проверка настроек доступа (чтение)" |
...
Сценарий: 18.26 Тест интерфейса для пользователя "Бюджетирование1" - Регламент подготовки отчетности
...
Сценарий: 18.27 Тест интерфейса для пользователя "Бюджетирование1" - Сценарии
	...
	И я перехожу к закладке с именем 'СвязанныеРегламенты'
	И я перехожу к закладке с именем 'ОсновныеСвойства'
	// ДОДЕЛАТЬ: Проверка на то, что форма доступна только для просмотра.		
	И Я закрываю окно "Сценарии: ВА - Основной сценарий (Сценарии)"

Сценарий: 18.28 Тест интерфейса для пользователя "Бюджетирование1" - Организации
...
Сценарий: 18.29 Тест интерфейса для пользователя "Бюджетирование1" - Периоды
	...
	И в таблице 'Список' я перехожу к строке:
		| "Дата начала" | "Дата окончания" | "Наименование" | "НСИ не активный" | "Периодичность" | "Произвольный" |
		| "01.01.2024"  | "31.12.2024"     | "2024 г."      | "Нет"             | "Год"           | "Нет"          |
	И в таблице 'Список' я выбираю текущую строку
	Тогда открылось окно "2024 г. (Периоды)"
	И Я закрываю окно "2024 г. (Периоды)"

Сценарий: 18.30 Тест интерфейса для пользователя "Бюджетирование1" - Виды аналитик (корпоративные)	
...
Сценарий: 18.31 Тест интерфейса для пользователя "Бюджетирование1" - Произвольный классификатор
...
Сценарий: 18.32 Тест интерфейса для пользователя "Бюджетирование1" - Проекты и этапы		
...
Сценарий: 18.33 Тест интерфейса для пользователя "Бюджетирование1" - Отчеты
...
Сценарий: 18.34 Тест интерфейса для пользователя "Бюджетирование1" - Сервис
		
	И я подключаю TestClient "УХ - Бюджетирование" логин "Бюджетирование1" пароль ''

	* Копирование данных показателей отчетов
		И В командном интерфейсе я выбираю "Бюджетирование, отчетность и анализ" "Копирование данных показателей отчетов"
	...
	* Сравнение табличных документов	
		...
		И В панели разделов я выбираю "Бюджетирование, отчетность и анализ"
