...
Сценарий: 07.03 Создание вида отчета "ВА - Нечисловые показатели (приемник)"
	...
	* Проверяем и меняем итоги
		...
		Тогда элемент формы с именем 'ВидИтога' стал равен "Итог по группе раскрытия не рассчитывается"
		И выпадающий список с именем 'ВидИтога' содержит значения
			| "Итог по группе раскрытия не рассчитывается" |
			| "Минимум" |
			| "Максимум" |	
		И из выпадающего списка с именем 'ВидИтога' я выбираю точное значение "Максимум"	
		Тогда элемент формы с именем 'СпособРасчетаИтогаПоПериоду' стал равен "Итог по периоду не рассчитывается"
		И выпадающий список с именем 'СпособРасчетаИтогаПоПериоду' содержит значения
			| "Итог по периоду не рассчитывается" |		
...
Сценарий: 07.05 Создание вида отчета "ВА - Группировка по аналитикам"
	...
	* Меняем настройки показателей
		* Сумма
			И я в конструкторе отчета в ячейке 'R2C2' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Сумма (оборот)"
			И я в конструкторе отчета в ячейке 'R3C2' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Сумма (оборот)"
		...
		* Максимум
			...
			И я в конструкторе отчета в ячейке 'R3C3' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Сумма (оборот)"
		...
		* Минимум
			И я в конструкторе отчета в ячейке 'R2C4' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Итог по периоду не рассчитывается"
			И я в конструкторе отчета в ячейке 'R3C4' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Сумма (оборот)"
			...
			И я в конструкторе отчета в ячейке 'R3C4' меняю свойство показателя 'ВидИтога' на "Минимум"
		* Среднее
			И я в конструкторе отчета в ячейке 'R3C5' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Сумма (оборот)"
		...
		* БезИтога
			...
			И я в конструкторе отчета в ячейке 'R3C6' меняю свойство показателя 'СпособРасчетаИтогаПоПериоду' на "Сумма (оборот)"
...
Сценарий: 07.07 Создание экземпляра отчета - "ВА - Нечисловые показатели (источник)" 
	...
	* Сравниваем итоговый документ
		...
		Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен:
			...
			| ''                                                                | "Январь 2024 г." | ''                | ''               | ''        | "Февраль 2024 г." | ''                | ''               | ''        | "Март 2024 г." | ''                | ''               | ''        | "ИТОГО" | ''                | ''               | ''        |
			| ''                                                                | "Цена"           | "ДатаПоступления" | "ВключатьВПрайс" | "Артикул" | "Цена"            | "ДатаПоступления" | "ВключатьВПрайс" | "Артикул" | "Цена"         | "ДатаПоступления" | "ВключатьВПрайс" | "Артикул" | "Цена"  | "ДатаПоступления" | "ВключатьВПрайс" | "Артикул" |
			| "Товары"                                                          | '1 206 000'      | '01.01.2024'      | "Нет"            | ''        | '1 326 600'       | '02.01.2024'      | "Нет"            | ''        | '1 326 600'    | '02.01.2024'      | "Нет"            | ''        | '0'     | ''                | "Нет"            | ''        |
			| "1С:Управление холдингом 8 "                                      | '1 250 000'      | '01.01.2024'      | "Да"             | '000001'  | '2 145 000'       | '02.05.2024'      | "Да"             | '2000005' | '2 145 000'    | '02.05.2024'      | "Да"             | '2000005' | '0'     | ''                | "Нет"            | ''        |
			...
			| "1С:ERP. Управление холдингом "                                   | '1 950 000'      | '01.04.2024'      | "Да"             | '000004'  | '660 000'         | '02.01.2024'      | "Да"             | '2000001' | '660 000'      | '02.01.2024'      | "Да"             | '2000001' | '0'     | ''                | "Нет"            | ''        |
