...
Сценарий: 07.04 Интерактивное удаление показателей
	...
	* Удаляем строку из формы раскрытия
		...
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетОтобразитьРаскрытие'
		Тогда открылось окно "Раскрытие показателей*"
		И в таблице "РаскрытиеПоказателей" я перехожу к строке:
	...
	* Сверяем результат
		Тогда табличный документ "ПолеТабличногоДокументаМакет" равен:
			...
			| ''                                      | 'Январь 2021 г.' | ''                | ''               | ''        | 'Февраль 2021 г.' | ''                | ''               | ''        | 'Март 2021 г.' | ''                | ''               | ''        | 'ИТОГО' | ''                | ''               | ''        |
			| ''                                      | 'Цена'           | 'ДатаПоступления' | 'ВключатьВПрайс' | 'Артикул' | 'Цена'            | 'ДатаПоступления' | 'ВключатьВПрайс' | 'Артикул' | 'Цена'         | 'ДатаПоступления' | 'ВключатьВПрайс' | 'Артикул' | 'Цена'  | 'ДатаПоступления' | 'ВключатьВПрайс' | 'Артикул' |
			| 'Товары'                                | '1 950 000'      | ''                | 'Нет'            | ''        | '1 686 667'       | '02.01.2021'      | 'Нет'            | ''        | '1 686 667'    | '02.01.2021'      | 'Нет'            | ''        | '0'     | '02.01.2021'      | 'Нет'            | ''        |
			| '1С:Управление холдингом 8 '            | '0'              | ''                | 'Нет'            | ' '       | '2 145 000'       | '02.05.2021'      | 'Да'             | '2000005' | '2 145 000'    | '02.05.2021'      | 'Да'             | '2000005' | '0'     | '02.05.2021'      | 'Нет'            | ''        |
			| '1С:Корпорация '                        | '0'              | ''                | 'Нет'            | ''        | '2 255 000'       | ''                | 'Нет'            | ''        | '2 255 000'    | ''                | 'Нет'            | ''        | '0'     | ''                | 'Нет'            | ''        |
			| '1С:ERP. Управление холдингом '         | '1 950 000'      | ''                | 'Да'             | '000004'  | '660 000'         | '02.01.2021'      | 'Да'             | '2000001' | '660 000'      | '02.01.2021'      | 'Да'             | '2000001' | '0'     | '02.01.2021'      | 'Нет'            | ''        |
