...
Сценарий: 14.01 Создание вида отчета "ВА - Протокол расчета"
	...
	* Вводим формулы расчета
	...
	И Я Для вида отчета "ВА - Протокол расчета" я создаю бланк по умолчанию
	И Я для вида отчета "ВА - Протокол расчета" я создаю бланк сводной таблицы по умолчанию с отборами ''	

Сценарий: 14.02 Создание экземпляра отчета "ВА - Протокол расчета" для организации "Меркурий ООО"	

	И Я создаю экземпляр отчета для вида отчета "ВА - Протокол расчета" сценарий "ВА - Основной сценарий" период '01.01.2021' '31.03.2021' периодичность "Месяц" организация "Меркурий ООО" проект '' аналитики '' '' '' '' '' ''
...
Сценарий: 14.03 Создание экземпляра отчета "ВА - Протокол расчета" для организации "Земля ООО"	

	И Я создаю экземпляр отчета для вида отчета "ВА - Протокол расчета" сценарий "ВА - Основной сценарий" период '01.01.2021' '31.03.2021' периодичность "Месяц" организация "Земля ООО" проект '' аналитики '' '' '' '' '' ''
...
Сценарий: 14.04 Выполняем расчет экземплятров отчетов из списка
	...
	* Проверяем расчет для "Меркурий ООО"
		...
		И в таблице 'Список' я перехожу к строке:
			| "Валюта отчета" | "Вид отчета"            | "Организация"  | "Период начала"  | "Период окончания" | "Сценарий"               |
			| 'RUB'           | "ВА - Протокол расчета" | "Меркурий ООО" | "Январь 2021 г." | "Март 2021 г."     | "ВА - Основной сценарий" |
	...
	* Проверяем расчет для "Меркурий ООО"
		...
		И в таблице 'Список' я перехожу к строке:
			| "Валюта отчета" | "Вид отчета"            | "Организация" | "Период начала"  | "Период окончания" | "Сценарий"               |
			| 'RUB'           | "ВА - Протокол расчета" | "Земля ООО"   | "Январь 2021 г." | "Март 2021 г."     | "ВА - Основной сценарий" |
		И в таблице 'Список' я выбираю текущую строку
...
Сценарий: 14.05 Выполняем расчет экземплятров отчетов из списка
	...
	* Открываем сводную таблицу
		...
		И Я устанавливаю отборы сводной таблицы: дата начала '01.01.2021', дата конца '31.03.2021', валюта 'RUB', сценарий "ВА - Основной сценарий", организация "Земля ООО"		
	...
	* Сверяем результат
		Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен:
			| "ВА - Протокол расчета (сводная таблица)" | "Январь 2021 г." | "Февраль 2021 г." | "Март 2021 г." | "ИТОГО" |
