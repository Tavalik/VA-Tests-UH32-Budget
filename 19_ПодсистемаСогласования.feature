...
Функционал: Тестирование подсистемы согласования
...
Сценарий: 19.01 Создание ролей
	...
	* Создать роли и добавить их в матрицу адресации
		...
		И Я добавляю строку в матрицу адресации роль 'ВА - Адресация по роли' пользователь 'Согласование2'
		И Я добавляю строку в матрицу адресации роль 'ВА - Роль расширенной адресации' пользователь 'Согласование3'
	...
	* Создать роль Суперпользователя
		...
		И Я добавляю строку в матрицу адресации роль 'Суперпользователь' пользователь 'Согласование4'

Сценарий: 19.02 Настройка документа ИБ	

Сценарий: 19.03 Создание вида отчета
...
Сценарий: 19.04 Создание экземпляра отчета

	И Я создаю экземпляр отчета для вида отчета "ВА - Согласование" сценарий "ВА - Основной сценарий" период '01.01.2021' '31.03.2021' периодичность "Месяц" организация "Меркурий ООО" проект '' аналитики '' '' '' '' '' ''
...
Сценарий: 19.05 Создание шаблона процесса согласования
	...
	* Удаляем существующие шаблоны
		...
		И пока в таблице 'Список' количество строк ">" 0 Тогда
			...
			И Я запоминаю значение выражения '"ВА - (удалить) " + StrReplace(Новый УникальныйИдентификатор, "-", "")' в переменную 'УИД_Наименование'
			И в поле с именем 'Наименование' я ввожу значение переменной 'УИД_Наименование'
			И я нажимаю на кнопку с именем 'ФормаЗаписать'
			Тогда открылось окно "$УИД_Наименование$ (Шаблон универсального процесса)"
			И я нажимаю на кнопку с именем 'ФормаУстановитьПометкуУдаления'
			Тогда открылось окно "1С:Предприятие"
			И я нажимаю на кнопку с именем 'Button0'
			Тогда открылось окно "$УИД_Наименование$ (Шаблон универсального процесса)"
			И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
			И я жду закрытия окна "$УИД_Наименование$ (Шаблон универсального процесса)" в течение 20 секунд
			Тогда открылось окно "Редактирование этапов процесса: ВА - Шаблон процесса согласования"
	...
	* Настраиваем схему процесса		
		И Я добавляю этап согласования с наименованием 'Этап согласования 1' способ адресации 'Пользователь' пользователь 'Согласование1' роль '' расширенная адресация ''
		И Я добавляю этап согласования с наименованием 'Этап согласования 2' способ адресации 'Роль' пользователь 'Согласование2' роль 'ВА - Адресация по роли' расширенная адресация ''
		И Я добавляю этап согласования с наименованием 'Этап согласования 3' способ адресации 'Расширенная адресация' пользователь 'Согласование3' роль '' расширенная адресация 'ВА - Расширенная адресация'
		И Я добавляю этап согласования с наименованием 'Этап согласования 4' способ адресации 'Пользователь' пользователь 'Согласование4' роль '' расширенная адресация ''
		...
		И Я добавляю связь этапа 'Этап согласования 1' с этапом 'Этап согласования 2'
...
Сценарий: 19.06 Настройка матрицы полномочий
...
Сценарий: 19.07 Сохранение настроек формы списка экземпляров отчетов	
	...
	* Установим фильтры
		...
		И Я в списке "Периоды" по полю "Наименование" ищу и выбираю элемент "Январь 2021 г." "По началу строки"
	...
	* Настраиваем пользователей
		...
		И в таблице "" я выбираю текущую строку
		И Я в списке "Выбор группы пользователей" по полю "Наименование" ищу и выбираю элемент "Согласование" "По точному совпадению"
		Тогда открылось окно "ВА - Согласование - Меркурий ООО (Вариант настройки) *"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна "ВА - Согласование - Меркурий ООО (Вариант настройки) *" в течение 20 секунд				

Сценарий: 19.08 Согласование по пользователеми

	* Согласование 1
		И я закрываю TestClient "УХ - Бюджетирование"
		И я подключаю TestClient "УХ - Бюджетирование" логин "Согласование1" пароль ''
		...
		И табличный документ 'ТабДокПроцесса' содержит строки из макета "Макеты\ВА_ТестированиеСогласованияЭтап1_Меркурий.mxl" по шаблону
		И Я закрываю окно 'Согласование: Экземпляр отчета * от *'
		...
		Тогда открылось окно '$ЗаголовокОкна$'
		И я нажимаю на кнопку с именем 'МаршрутСогласования'
		И табличный документ 'ТабДокПроцесса' содержит строки из макета "Макеты\ВА_ТестированиеСогласованияЭтап2_Меркурий.mxl" по шаблону
		И Я закрываю окно 'Согласование: Экземпляр отчета * от *'

	* Согласование2
		И я закрываю TestClient "УХ - Бюджетирование"
		И я подключаю TestClient "УХ - Бюджетирование" логин "Согласование2" пароль ''
		...
		Тогда открылось окно '$ЗаголовокОкна$'
		И я нажимаю на кнопку с именем 'МаршрутСогласования'
		И табличный документ 'ТабДокПроцесса' содержит строки из макета "Макеты\ВА_ТестированиеСогласованияЭтап3_Меркурий.mxl" по шаблону
		И Я закрываю окно 'Согласование: Экземпляр отчета * от *'

	* Согласование3
		И я закрываю TestClient "УХ - Бюджетирование"
		И я подключаю TestClient "УХ - Бюджетирование" логин "Согласование3" пароль ''
		...
		Тогда открылось окно '$ЗаголовокОкна$'
		И я нажимаю на кнопку с именем 'МаршрутСогласования'
		И табличный документ 'ТабДокПроцесса' содержит строки из макета "Макеты\ВА_ТестированиеСогласованияЭтап4_Меркурий.mxl" по шаблону
		И Я закрываю окно 'Согласование: Экземпляр отчета * от *'

	* Согласование4
		И я закрываю TestClient "УХ - Бюджетирование"
		И я подключаю TestClient "УХ - Бюджетирование" логин "Согласование4" пароль ''
		...
		Тогда открылось окно '$ЗаголовокОкна$'
		И я нажимаю на кнопку с именем 'МаршрутСогласования'
		И табличный документ 'ТабДокПроцесса' содержит строки из макета "Макеты\ВА_ТестированиеСогласованияАвтоутверждение_Меркурий.mxl" по шаблону
		И Я закрываю окно 'Согласование: Экземпляр отчета * от *'
	...
	* История согласования
		...
		И табличный документ 'Результат' равен макету "Макеты\ВА_ТестированиеСогласованияИстория_Меркурий.mxl" по шаблону			
