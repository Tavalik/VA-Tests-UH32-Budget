...
Сценарий: 03.04 Создание экземпляра отчета - "ВА - ИмпортExcel"	и загрузка из файла
	...
	* Загружаем ошибочный файл
		...
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Ошибка.xlsx"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'		
	...
	* Загружаем с файлом
		...
		Тогда у элемента формы с именем 'СпособФормированияОтчета' текст редактирования стал равен "Импорт"
		Тогда у элемента формы с именем 'БланкИмпорта' текст редактирования стал равен "ВА - ИмпортExcel"
		Тогда у элемента формы с именем 'ФайлИмпорта' текст редактирования стал равен "ВА_ИмпортExcel_Ошибка.xlsx"
		И я меняю значение переключателя с именем 'СпособИмпортирования' на 'Файл на диске'
		Тогда у элемента формы с именем 'ПутьКФайлу' текст редактирования стал равен "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Ошибка.xlsx"
	...
	* Настраиваем Регламент
		...
		* Настраиваем файл импорта
			Когда открылось окно "Настройка правил заполнения отчетов"
			И я нажимаю на кнопку открытия поля с именем 'ВнешнийИсточник'
			...
			И из выпадающего списка с именем 'Объект' я выбираю по строке "ВА - Импорт из Excel"
			И в поле с именем 'Имя' я ввожу текст 'ВА_ИмпортExcel_Меркурий.xlsx'
			И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
...
Сценарий: 03.05 Повторная загрузка данных из файла
	...
	* Заполняем из внешней базы
		...
		Когда открылось окно "Выбор способа расчета показателей"
		И я нажимаю на кнопку открытия поля с именем 'ИспользуемаяИБИмпорт'
		Тогда открылось окно "ВА - Импорт из Excel (Внешние информационные базы)"
		И я нажимаю на кнопку с именем 'ОткрытьНастройкиОбменаФайламиОрганизаций'
		Тогда открылось окно "Элементы пути к файлам организаций: Форма списка"
		И я перехожу к закладке с именем 'Страница_ВидОтчета'
		...
		Если в таблице 'Список_ВидОтчета' количество строк 'больше' 0 Тогда
			И в таблице 'Список_ВидОтчета' я выбираю текущую строку
		Иначе	
			И в таблице 'Список_ВидОтчета' я нажимаю на кнопку с именем 'Список_ВидОтчетаСоздать'
		Тогда открылось окно "Элементы пути к файлам организаций*"
		И из выпадающего списка с именем 'Объект' я выбираю по строке "ВА - ИмпортExcel"
		И в поле с именем 'Имя' я ввожу текст 'ВА_ИмпортExcel_Меркурий.xlsx'
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		Когда открылось окно "Элементы пути к файлам организаций: Форма списка"
		И Я закрываю окно "Элементы пути к файлам организаций: Форма списка"
		Тогда открылось окно "ВА - Импорт из Excel (Внешние информационные базы)"
		И Я закрываю окно "ВА - Импорт из Excel (Внешние информационные базы)"
