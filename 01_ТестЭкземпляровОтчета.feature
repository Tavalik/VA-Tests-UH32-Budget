...
Сценарий: 01.11 Проверяем движения документа
	...
	* Проверяем движения документа
		...
		Дано Табличный документ 'ОтчетТабличныйДокумент' равен макету '\\Макеты\01\ВА_ВидОтчетаСАналитиками_Меркурий_Движения.mxl' по шаблону
...
Сценарий: 01.12 Копирование экземпляра отчета
	...
	* Проверяем движения документа
		...
		Тогда открылось окно "Плоская таблица значений показателей"
		И я жду когда в табличном документе 'ОтчетТабличныйДокумент' заполнится ячейка 'R2C1' в течение 30 секунд
		Дано Табличный документ 'ОтчетТабличныйДокумент' равен макету '\\Макеты\01\ВА_ВидОтчетаСАналитиками_Венера_Движения.mxl' по шаблону
		И Я закрываю окно "Плоская таблица значений показателей"
...
Сценарий: 01.13 Консолидация периметра
	...
	* Создаем документ
		И Я создаю экземпляр отчета для вида отчета "ВА - Ручной ввод по всем аналитикам" сценарий "ВА - Основной сценарий" период '01.01.2024' '31.03.2024' периодичность "Месяц" организация "Система ООО" проект "ВА - Основной проект" аналитики '' '' '' '' '' ''
