...
Сценарий: 06.02 Создание вида отчета 'ВА - Данные для дополнения аналитик'	
	...
	И Я Для вида отчета 'ВА - Данные для дополнения аналитик' я создаю бланк по умолчанию

Сценарий: 06.03 Создание экзмемпляра отчета 'ВА - Данные для дополнения аналитик'

	И Я создаю экземпляр отчета для вида отчета 'ВА - Данные для дополнения аналитик' сценарий 'ВА - Основной сценарий' период '01.01.2021' '31.03.2021' периодичность "Месяц" организация 'Меркурий ООО' проект '' аналитики 'Программные продукты' '' '' '' '' '' 
...
Сценарий: 06.04 Создание вида отчета 'ВА - Аналитики из полей других источников'
	...
	* Устанавливаем формулы расчета
		...
		* Другой источник
			...
			* Источник с прозвольным запросом №1
				...
				И в таблице 'Список' я перехожу к строке:
					| "Наименование"                                          | "Способ получения"                                 | "Тип ИБ"                      | "Показатель отбор"  | "Код"                                                |
					| 'ВА - Данные для дополнения аналитик_Товары количество' | "Показатель отчета текущей ИБ (простая настройка)" | "Текущая информационная база" | 'Товары количество' | 'ВАДанныеДляДополненияАналитик_Товары_Количество01 ' |
				И в таблице 'Список' я выбираю текущую строку				
				Тогда открылось окно "Источник данных (создание)*"
				И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
				И я жду закрытия окна "Источник данных (создание)*" в течение 20 секунд
	...
	И Я Для вида отчета 'ВА - Аналитики из полей других источников' я создаю бланк по умолчанию
	...
	И Я Для вида отчета 'ВА - Аналитики из полей других источников' в бланке для группы раскрытия с адресом 'R13C1' задаю сортировку 'Номенклатура.Артикул'
	И Я Для вида отчета 'ВА - Аналитики из полей других источников' в бланке для группы раскрытия с адресом 'R19C1' задаю сортировку 'Номенклатура.Артикул'

Сценарий: 06.05 Создание экзмемпляра отчета 'ВА - Аналитики из полей других источников'

	И Я создаю экземпляр отчета для вида отчета 'ВА - Аналитики из полей других источников' сценарий 'ВА - Основной сценарий' период '01.01.2021' '31.03.2021' периодичность "Месяц" организация 'Меркурий ООО' проект '' аналитики 'Программные продукты' '' '' '' '' ''
	И я запоминаю текущее окно как 'ЗаголовокОкна' 
