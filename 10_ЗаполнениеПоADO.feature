...
Сценарий: 10.02 Создаем экземпляр отчета
	...
	* Сверяем результат
		...
		// ДОДЕЛАТЬ: в текущей версии не отборажается итог по периоду по цене
		Если '$$ЭтоCPMWE$$' Тогда
			Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен:
				...
				| 'Строка1'                                                         | ''               | '36'         | '839 627'   | '15 965 000' | ''                | '61'         | '958 752'   | '34 723 000' | ''              | '86'         | '1 072 802' | '58 228 400' | ''        | '183'        | ''     | '108 916 400' |
