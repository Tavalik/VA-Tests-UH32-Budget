...
Сценарий: Я удаляю тип внешней информационной базы с именем 'TheExternalInfobaseName' 
	* Открываем список
		И В командном интерфейсе я выбираю "Интеграция и управление мастер-данными" "Внешние информационные базы"
	* Удаляем текущий элемент	
		Тогда открылось окно "Внешние информационные базы"
	...
	* Удаляем тип ВИБ
		...
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			...
			* Переименовываем
				И Я запоминаю значение выражения 'СтрЗаменить(Новый УникальныйИдентификатор, "-", "")' в переменную 'УИД'	
				И в таблице 'Список' я выбираю текущую строку
				Тогда открылось окно '[TheExternalInfobaseName] (Тип информационной базы)'
				И в поле с именем 'Наименование' я ввожу текст 'УИД'
				И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
				И я жду закрытия окна '[TheExternalInfobaseName] (Тип информационной базы) *' в течение 20 секунд		