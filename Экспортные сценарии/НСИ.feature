...
Сценарий: Я для сценария 'TheName' на закладке 'ThePage' для реквизита 'TheProperty' устанавливаю значение 'TheValue' 
	...
	* Ищем элемент	
		И В командном интерфейсе я выбираю "Бюджетирование, отчетность и анализ" "Сценарии"
		И Я в списке "Сценарии" по полю "Наименование" ищу элемент '[TheName]' "По точному совпадению"
...
Сценарий: Я создаю вид аналитики с кодом 'TheAnalyticsCode' именем 'TheAnalyticsName' и типом 'TheAnalyticsType'
	...
	* Настраиваем вид аналитики
		...
		ИначеЕсли в таблице 'ТабличноеПолеРеквизиты' есть строка Тогда
			| "Реквизит"                 | "Ключ" | "Макет" |
			| "Наименование в программе" | "Нет"  | "Нет"   |
			И в таблице 'ТабличноеПолеРеквизиты' я перехожу к строке по шаблону:
				| "Реквизит"                 | "Ключ" | "Макет" |
				| "Наименование в программе" | "Нет"  | "Нет"   |
			И в таблице 'ТабличноеПолеРеквизиты' я изменяю флаг с именем 'ТаблицаРеквизитовМакет'
...
Сценарий: Я создаю тип информационной базы с именем 'TheInfobaseTypeName' с версией 'PlatformVersion'
	...
	* Удаляем тип ВИБ
		...
		И из выпадающего списка с именем 'FieldSelector' я выбираю точное значение "Наименование"
		И я меняю значение переключателя с именем 'CompareType' на "По началу строки"
		И в поле с именем 'Pattern' я ввожу текст '[TheInfobaseTypeName]'
		И я нажимаю на кнопку с именем 'Find'
		Тогда открылось окно "Типы информационных баз"
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			* Ставим пометку на удаление
			...
			* Переименовываем
				И Я запоминаю значение выражения 'СтрЗаменить(Новый УникальныйИдентификатор, "-", "")' в переменную 'УИД'	
				И в таблице 'Список' я выбираю текущую строку
				Тогда открылось окно '[TheInfobaseTypeName] (Тип информационной базы)'
				И в поле с именем 'Наименование' я ввожу текст 'УИД'
				И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
				И я жду закрытия окна '[TheInfobaseTypeName] (Тип информационной базы) *' в течение 20 секунд
	...
	* Создаем новый элемент
		...
		И в поле с именем 'Наименование' я ввожу текст '[TheInfobaseTypeName]'
		И из выпадающего списка с именем "ВерсияПлатформы" я выбираю точное значение '[PlatformVersion]'
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
...
Сценарий: Я создаю внешнюю информационную базу 'TheExternalInfobaseName' тип 'TheExternalInfobaseType' каталог 'TheCatalog' тип хранилища 'TheTypeOfStorage'
