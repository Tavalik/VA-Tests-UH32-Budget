...
Сценарий: Я для вида отчета 'TheReportKindName' изменяю флаг 'ТекИмяФлага' 
...
Сценарий: Я октрываю сводную таблицу отчета с именем 'TheReportKindName'
	...
	И я запоминаю строку '[TheReportKindName]' в переменную 'ИмяВидаОтчета'
...
Сценарий: Я Для вида отчета 'TheReportKindName' я создаю бланк для импорта по умолчанию
	...
	* Откроем вид отчета
		Если имя текущей формы "[TheReportKindName] (Виды отчетов)" Тогда
			// Форма уже открыта
		Иначе
			И Я открываю вид отчета с именем '[TheReportKindName]'
...
Сценарий: Я Для вида отчета 'TheReportKindName' я создаю бланк сводной таблицы по умолчанию с отборами 'ТекОтбор1' 'ТекОтбор2' 'ТекОтбор3'
...
Сценарий: Я для регламента 'ТекИмяРегламента' изменяю флаг 'ТекИмяФлага'
	...
	Тогда открылось окно "Регламенты подготовки отчетности"
	И в таблице 'Список' я выбираю текущую строку
	Тогда Открылся регламент подготовки отчетности 'ТекИмяРегламента'
...
Сценарий: Я создаю экземпляр отчета для вида отчета 'ТекВидОтчета' сценарий 'ТекСценарий' период 'ТекДатаНачала' 'ТекДатаОкончания' периодичность 'ТекПериодичность' организация 'ТекОрганизация' проект 'TheProject' аналитики 'TheDimension1' 'TheDimension2' 'TheDimension3' 'TheDimension4' 'TheDimension5' 'TheDimension6' 
	...
	* Заполняем ключевые реквизиты
		...
		И Открылся экземпляр отчета для вида отчета '[ТекВидОтчета]' валюта '*' организация '[ТекОрганизация]' сценарий '[ТекСценарий]' периодичность '[ТекПериодичность]' проект '[TheProject]' аналитики '[TheDimension1]' '[TheDimension2]' '[TheDimension3]' '[TheDimension4]' '[TheDimension5]' '[TheDimension6]' 	
