...
Сценарий: Я создаю страну с кодом1 "ТекКодСтраны1" кодом2 "ТекКодСтраны2" кодом3 "ТекКодСтраны3" именем1 "ТекИмяСтраны1" именем2 "ТекИмяСтраны2" именем3 "ТекИмяСтраны3" если ее нет в 1C:CPMWE
	...
	* Создаем элементм
		...
		И я жду закрытия окна '*Страна мира*' в течение 20 секунд

	* Закрываем список
		Когда открылось окно 'Страны мира'
		И Я закрываю окно 'Страны мира'

Сценарий: Я устанавливаю цену с типом "ТипЦены" номенклатуры "ТекИмяНоменклатуры" на дату "ТекДатаКурса" в значение "ТекЗначение" "ТекВалюта" в 1C:CPMWE
	...
	* Ищем текущий элемент
		...
		И из выпадающего списка с именем "Pattern" я выбираю по строке 'ТипЦены'
		И я нажимаю на кнопку с именем 'Find'
		Тогда открылось окно '* (Номенклатура)'		
		Если в таблице "Список" количество строк "равно" 0 Тогда
	...
	* Создаем позицию
		...
		И из выпадающего списка с именем "ТипЦен" я выбираю по строке 'ТипЦены'
