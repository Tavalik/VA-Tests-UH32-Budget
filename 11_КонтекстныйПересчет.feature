...
Сценарий: 11.01 Создаем вид отчета "ВА - Контекстный пересчет"
...
Сценарий: 11.03 Вводим данные в сводную таблицу	
	...
	* Вводим значения вручную
		...
		* Дата
			...
			//++ ДОДЕЛАТЬ
			// Не пересчитывается итог по дате
			//-- ДОДЕЛАТЬ
		* Строка
			Когда открылось окно 'Сводная таблица: ВА - Контекстный пересчет(cводная таблица)( Основной) *'
			...
			//++ ДОДЕЛАТЬ
			// Не отображается синтетическое строковое значение
			//-- ДОДЕЛАТЬ		
	...
	* Копирование и вставка значений
		...
		//++ ДОДЕЛАТЬ 
		// Не копируются нечисловые показатели
		Когда открылось окно 'Сводная таблица: ВА - Контекстный пересчет(cводная таблица)( Основной) *'
	...
	* Очистка показателей
		...
		//++ ДОДЕЛАТЬ
		// Макет должен быть полностью пустой
		//-- ДОДЕЛАТЬ
	...
	* Изменим значение и закроем не сохраняя
		Когда открылось окно 'Сводная таблица: ВА - Контекстный пересчет(cводная таблица)( Основной)'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке "R6C2"
		И в табличном документе 'ПолеТабличногоДокументаМакет' я делаю двойной клик на текущей ячейке
...
Сценарий: 11.04 Создаем новый сценарий для индексации
...
Сценарий: 11.05 Копируем все данные отчетного периода
...
Сценарий: 11.06 Копируем данные по показателям
...
Сценарий: 11.07 Проверяем сводную таблицу "ВА - Контекстный пересчет"
	...
	* Сравниваем результат
		Тогда табличный документ "ПолеТабличногоДокументаМакет" равен:
			...
			| 'Товары'                                     | ''                             | '0'              | '0'           | '0'    | ''     | ''       | '0,00013'         | '0,00003'     | '0,00027' | '01.01.2021' | ''        | '0,00040'      | '0,00008'     | '0,00242' | '01.01.2021' | ''        | '0,00054'           | '0,00011'     | '0,00269' | '01.01.2021' | ''        | '0,00054'    | '0,00011'     | '0,00269' | '01.01.2021' | ''       |
			| ''                                           | '1С:ERP. Управление холдингом' | '0'              | '0'           | '0'    | ''     | ''       | '0,00013'         | '0,00003'     | '0,00027' | '01.01.2021' | 'Строка1' | '0,00040'      | '0,00008'     | '0,00242' | '01.01.2021' | 'Строка1' | '0,00054'           | '0,00011'     | '0,00269' | '01.01.2021' | 'Строка1' | '0,00054'    | '0,00011'     | '0,00269' | '01.01.2021' | ''       |

Сценарий: 11.08 Индексируем по сценарию
	...
	* Индексируем
		Когда открылось окно 'Сводная таблица: ВА - Контекстный пересчет(cводная таблица)( Основной)*'
		//++ ДОДЕЛАТЬ
		// Не работает индексация, если выбрать такой диапазон
